...
![](E:\pro\web_preview\JavaScript相关\images\181511124714709.png)
...
![](E:\pro\web_preview\JavaScript相关\images\181637013624694.png)
...
![](E:\pro\web_preview\JavaScript相关\images\182012548152475.png)
...
![](E:\pro\web_preview\JavaScript相关\images\182013450814552.png)
...
![](E:\pro\web_preview\JavaScript相关\images\182014022217881.png)
...
![](E:\pro\web_preview\JavaScript相关\images\182014277067963.png)
...
![](E:\pro\web_preview\JavaScript相关\images\182015334711671.png)
...
![](E:\pro\web_preview\JavaScript相关\images\182102552534656.png)

其次，如果继承的方法不合适，可以做出修改。

![](E:\pro\web_preview\JavaScript相关\images\182103176287054.png)

如上图，Object和Array的toString()方法不一样。肯定是Array.prototype.toString()方法做了修改。

同理，我也可以自定义一个函数，并自己去修改prototype.toString()方法。

![](E:\pro\web_preview\JavaScript相关\images\182103437218549.png)

最后，如果感觉当前缺少你要用的方法，可以自己去创建。

例如在json2.js源码中，为Date、String、Number、Boolean方法添加一个toJSON的属性。

![](E:\pro\web_preview\JavaScript相关\images\182104105812645.png)
...
![](E:\pro\web_preview\JavaScript相关\images\221743466231490.png)
...
![](E:\pro\web_preview\JavaScript相关\images\221744084828533.png)

这是第一种情况。

下面还有。先来个简单的。

![](E:\pro\web_preview\JavaScript相关\images\221744319354566.png)
...
![](E:\pro\web_preview\JavaScript相关\images\221745066078791.png)
...
![](E:\pro\web_preview\JavaScript相关\images\221746370927602.png)

其次，eval代码接收的也是一段文本形式的代码。

![](E:\pro\web_preview\JavaScript相关\images\221747371078703.png)

最后，函数体是代码段是因为函数在创建时，本质上是 new Function(…) 得来的，其中需要传入一个文本形式的参数作为函数体。

![](E:\pro\web_preview\JavaScript相关\images\221746583578531.png)
...
![](E:\pro\web_preview\JavaScript相关\images\230840377174282.png)
...
![](E:\pro\web_preview\JavaScript相关\images\230841277489133.png)
...
![](E:\pro\web_preview\JavaScript相关\images\231451530452221.png)

以上代码中，如果函数作为构造函数用，那么其中的this就代表它即将new出来的对象。

注意，以上仅限new Foo()的情况，即Foo函数作为构造函数的情况。如果直接调用Foo函数，而不是new Foo()，情况就大不一样了。

![](E:\pro\web_preview\JavaScript相关\images\231452183579852.png)

这种情况下this是window，我们后文中会说到。

在构造函数的prototype中，this代表着什么。

![](E:\pro\web_preview\JavaScript相关\images\270954235926791.png)
...
![](E:\pro\web_preview\JavaScript相关\images\231452462177631.png)

以上代码中，fn不仅作为一个对象的一个属性，而且的确是作为对象的一个属性被调用。结果this就是obj对象。

注意，如果fn函数不作为obj的一个属性被调用，会是什么结果呢？

![](E:\pro\web_preview\JavaScript相关\images\231453370605897.png)
...
![](E:\pro\web_preview\JavaScript相关\images\231454121399180.png)

**情况4：全局 & 调用普通函数**

在全局环境下，this永远是window，这个应该没有非议。

![](E:\pro\web_preview\JavaScript相关\images\231454368571699.png)

普通函数在调用时，其中的this也都是window。

![](E:\pro\web_preview\JavaScript相关\images\231454563265514.png)

以上代码很好理解。

不过下面的情况你需要注意一下：

![](E:\pro\web_preview\JavaScript相关\images\231455260602664.png)
...
![](E:\pro\web_preview\JavaScript相关\images\231455536397656.png)
...
![](E:\pro\web_preview\JavaScript相关\images\232122300768665.png)

可根据以下代码来详细介绍上下文栈的压栈、出栈过程。

![](E:\pro\web_preview\JavaScript相关\images\232122515459124.png)

如上代码。

在执行代码之前，首先将创建全局上下文环境。

![](E:\pro\web_preview\JavaScript相关\images\232123126545539.png)

然后是代码执行。代码执行到第12行之前，上下文环境中的变量都在执行过程中被赋值。

![](E:\pro\web_preview\JavaScript相关\images\232123436078861.png)

执行到第13行，调用bar函数。

跳转到bar函数内部，执行函数体语句之前，会创建一个新的执行上下文环境。

![](E:\pro\web_preview\JavaScript相关\images\232124103579967.png)

并将这个执行上下文环境压栈，设置为活动状态。

![](E:\pro\web_preview\JavaScript相关\images\232124277955196.png)

执行到第5行，又调用了fn函数。进入fn函数，在执行函数体语句之前，会创建fn函数的执行上下文环境，并压栈，设置为活动状态。

![](E:\pro\web_preview\JavaScript相关\images\232124478267882.png)

待第5行执行完毕，即fn函数执行完毕后，此次调用fn所生成的上下文环境出栈，并且被销毁（已经用完了，就要及时销毁，释放内存）。

![](E:\pro\web_preview\JavaScript相关\images\232125095291412.png)

同理，待第13行执行完毕，即bar函数执行完毕后，调用bar函数所生成的上下文环境出栈，并且被销毁（已经用完了，就要及时销毁，释放内存）。

![](E:\pro\web_preview\JavaScript相关\images\232125295149083.png)
...
![](E:\pro\web_preview\JavaScript相关\images\241706483263428.png)

再比如for语句：

![](E:\pro\web_preview\JavaScript相关\images\241707060293268.png)

所以，我们在编写代码的时候，不要在“块”里面声明变量，要在代码的一开始就声明好了。以避免发生歧义。如：

![](E:\pro\web_preview\JavaScript相关\images\241707273423913.png)
...
![](E:\pro\web_preview\JavaScript相关\images\241708128263977.png)

下面继续说作用域。作用域是一个很抽象的概念，类似于一个“地盘”

![](E:\pro\web_preview\JavaScript相关\images\241708372951952.png)
...
![](E:\pro\web_preview\JavaScript相关\images\241709349512366.png)
...
![](E:\pro\web_preview\JavaScript相关\images\250813253577626.png)
...
![](E:\pro\web_preview\JavaScript相关\images\250814158269779.png)

第二步，程序执行到第27行，调用fn(10)，此时生成此次调用fn函数时的上下文环境，压栈，并将此上下文环境设置为活动状态。

![](E:\pro\web_preview\JavaScript相关\images\250814386853995.png)

第三步，执行到第23行时，调用bar(100)，生成此次调用的上下文环境，压栈，并设置为活动状态。

![](E:\pro\web_preview\JavaScript相关\images\250815006238997.png)

第四步，执行完第23行，bar(100)调用完成。则bar(100)上下文环境被销毁。接着执行第24行，调用bar(200)，则又生成bar(200)的上下文环境，压栈，设置为活动状态。

![](E:\pro\web_preview\JavaScript相关\images\250815248579200.png)

第五步，执行完第24行，则bar(200)调用结束，其上下文环境被销毁。此时会回到fn(10)上下文环境，变为活动状态。

![](E:\pro\web_preview\JavaScript相关\images\250815435609914.png)

第六步，执行完第27行代码，fn(10)执行完成之后，fn(10)上下文环境被销毁，全局上下文环境又回到活动状态。

![](E:\pro\web_preview\JavaScript相关\images\250816112012394.png)

结束了。像老太太的裹脚布——又臭又长！

最后我们可以把以上这几个图片连接起来看看。

![](E:\pro\web_preview\JavaScript相关\images\250816269984619.png)
...
![](E:\pro\web_preview\JavaScript相关\images\251447094515754.png)

如上程序中，在调用fn()函数时，函数体中第6行。取b的值就直接可以在fn作用域中取，因为b就是在这里定义的。而取x的值时，就需要到另一个作用域中取。到哪个作用域中取呢？

有人说过要到父作用域中取，其实有时候这种解释会产生歧义。例如：

![](E:\pro\web_preview\JavaScript相关\images\251447377485132.png)
...
![](E:\pro\web_preview\JavaScript相关\images\251448515607115.png)
...
![](E:\pro\web_preview\JavaScript相关\images\260747286391027.png)

如上代码，bar函数作为返回值，赋值给f1变量。执行f1(15)时，用到了fn作用域下的max变量的值。至于如何跨作用域取值，可以参考上一节。

**第二，函数作为参数被传递**

![](E:\pro\web_preview\JavaScript相关\images\260747498738414.png)
...
![](E:\pro\web_preview\JavaScript相关\images\260749156077205.png)

第一步，代码执行前生成全局上下文环境，并在执行时对其中的变量进行赋值。此时全局上下文环境是活动状态。

![](E:\pro\web_preview\JavaScript相关\images\260749349988764.png)

第二步，执行第17行代码时，调用fn()，产生fn()执行上下文环境，压栈，并设置为活动状态。

![](E:\pro\web_preview\JavaScript相关\images\260750319351092.png)
...
![](E:\pro\web_preview\JavaScript相关\images\260957500455644.png)

第四步，执行到第20行，执行f1(15)，即执行bar(15)，创建bar(15)上下文环境，并将其设置为活动状态。

![](E:\pro\web_preview\JavaScript相关\images\260958057327369.png)
